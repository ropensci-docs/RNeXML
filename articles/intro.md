...
Pretty-print an overview of the components and metadata that make up the
nexml object:
...
Create a summary object of various component and metadata counts (the
[`show()`](https://rdrr.io/r/methods/show.html) method uses this):
...
Extract trees from nexml into the
[`ape::phylo`](https://rdrr.io/pkg/ape/man/read.tree.html) format:
...
Write an [`ape::phylo`](https://rdrr.io/pkg/ape/man/read.tree.html) tree
into the `nexml` format:
...
attribute as a homepage, a term in the `foaf` vocabulary. Because `foaf`
is not a default namespace, we will have to provide its URL in the full
definition below.
